...
import { getGroupActivity, getGroupMemberships, getGroupSummary } from '@/api/groups';
...
export function useGroupDetail(groupId: string | null) {
  ...
  const load = useCallback(async (refresh: boolean) => {
    ...
    try {
      const [summaryResponse, membershipsResponse, activityResponse] = await Promise.all([
      ...
      setData({ ...parsed, members: parseMembershipsResponse(membershipsResponse), activity: parseActivityResponse(activityResponse) });
    } catch (requestError) {
